...
import { CheckCircle, XCircle, Building2, Hourglass, Star, MessageSquare } from 'lucide-react';
...
export default function AdminPGsPage() {
  ...
  useEffect(() => {
    ...
    if (user) fetch(searchParams.get('approved') === '0' ? 'pending' : searchParams.get('approved') === '1' ? 'approved' : 'all');
                ...
                <tr key={pg.id}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                      <Link href={`/chat/${pg.id}?other_user_id=${pg.owner_id}`} className="btn btn-ghost btn-sm" style={{ minWidth: '112px' }}>
